/**
 * Service de notifications email pour les demandes de rendez-vous (projets)
 * - Envoi au tatoueur (nouvelle demande)
 * - Envoi au client (confirmation)
 * - Retry automatique 1x après 30s, mise à jour DB en cas d'échec définitif
 */
...
export type SendAppointmentNotificationOptions = {
  ...
  description: string;
  siteBaseUrl: string;
  /** Callback appelé en cas d'échec définitif pour sauvegarder "email_failed" en DB */
  onEmailFailed?: (projectId: string) => Promise<void>;
};

/**
 * Envoie l'email "Nouvelle demande de rendez-vous" au tatoueur.
 * - Try/catch autour de l'envoi
 * - En cas d'échec : log détaillé, retry 1x après 30s
 * - Si échec définitif : appelle onEmailFailed(projectId) pour sauvegarder l'état en DB
 * - Ne lance jamais : ne bloque pas la création du rendez-vous
 */
export async function sendAppointmentNotification(options: SendAppointmentNotificationOptions): Promise<NotificationResult> {
  ...
  try {
    ...
    console.error('[appointmentNotification] Unexpected error sending artist email:', {
      projectId,
      message,
      stack,
    });
...
/**
 * Envoie l'email de confirmation au client (bonus).
 * Pas de retry ni mise à jour DB pour ce mail.
 */
